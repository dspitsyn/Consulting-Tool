/*
	...
	| Version:  2001.371109 - January, 2020
	| Summary:  	
	| This script will create MAINTENANCE_ALL_DATABASES job incl. standard steps (State: 26 January 2020) which can be used in any environment
	| SharePoint instances will be recognized automatically by Collation on Instance level while execution
	| Configure and schedule regular maintenance for all of the following:
	| • ST01: CHECK_INTEGRITY_ALL_DATABASES
	| • ST02: OPTIMIZE_INDEX_ALL_DATABASES
	| • ST03: OPTIMIZE_INDEX_MSDB
	| • ST04: CLEANUP_BACKUP_HISTORY
	| • ST05: CLEANUP_TABLE_COMMANDLOG
	| • ST06: CLEANUP_JOB_HISTORY
	| • ST07: CLEANUP_MESSAGES
	| • ST08: CLEANUP_OUTPUT_TXTFILES
	| • ST09: CLEANUP_OUTPUT_AUDITFILES
	| • ST10: CHECK_STEPS_ERRORS

	| Job CYCLE_LOG - it will automatically rotating the SQL Server Error Log
	| Job STOP_EXECUTION - Job can be manually started to stop running Maintenance Job.
	|	Some long-running Transact-SQL statements such as BACKUP, RESTORE, and some DBCC commands can take a long time to finish.

	| Optional:
	| • System databases backup
	| • Full user databases backup
	| • Log backups (for databases in Full recovery model)
	| • Create Profile and Database Mail account
	| • Create operator

	| Version Updates:
		10.09.2020 | Added by Dmitry Spitsyn
...
*/
	...
	*/
	...
	DECLARE @sExecutionTime NVARCHAR(MAX) = LEFT(@JobExecutionTime, LEN(@JobExecutionTime) - 8) + ' PM';

	DECLARE @OutputFileDirectory NVARCHAR(MAX)
		, @LogToTable NVARCHAR(MAX)
		, @DatabaseName NVARCHAR(MAX)
		, @Version NUMERIC(18,10)
		, @TokenServer NVARCHAR(MAX)
		, @TokenJobID NVARCHAR(MAX)
		, @TokenStepID NVARCHAR(MAX)
		, @TokenDate NVARCHAR(MAX)
		, @TokenTime NVARCHAR(MAX)
		, @JobDescription NVARCHAR(MAX)
		, @JobDescriptionStopExecution NVARCHAR(MAX)
		, @JobDescriptionCyclingErrorLog NVARCHAR(MAX)
		, @JobCategory NVARCHAR(MAX)
		, @JobOwner NVARCHAR(MAX)
		, @JobNameMaintenanceSolution NVARCHAR(MAX)
		, @JobNameOldCyclingErrorLog VARCHAR(MAX)
		, @JobNameCyclingErrorLog NVARCHAR(MAX)
		, @ServiceAccount NVARCHAR(MAX)
		, @JobNameStopExecution NVARCHAR(MAX)
		, @JobNameBackupSystem NVARCHAR(MAX)
		, @JobNameBackupUser NVARCHAR(MAX)
		, @JobNameBackupUserLog NVARCHAR(MAX)
		, @JobCommandIntegrityCheck NVARCHAR(MAX)
		, @JobCommandIndexOptimize NVARCHAR(MAX)
		, @JobCommandSPSIndexOptimize NVARCHAR(MAX)
		, @JobCommandMSDBIndexOptimize NVARCHAR(MAX)
		, @JobCommandCleanupTableCommandLog NVARCHAR(MAX)
		, @JobCommandPurgeHistory NVARCHAR(MAX)
		, @JobCommandCleanUpJobsHistory NVARCHAR(MAX)
		, @JobCommandCleanUpMessages NVARCHAR(MAX)
		, @JobCommandOutputTFileCleanUp NVARCHAR(MAX)
		, @JobCommandOutputAFileCleanUp NVARCHAR(MAX)
		, @JobCommandCheckStepsErrors NVARCHAR(MAX)
		, @JobCommandStopExecution NVARCHAR(MAX)
		, @JobCommandCyclingErrorLog NVARCHAR(MAX)
		, @JobCommandBackupSystem NVARCHAR(MAX)
		, @JobCommandBackupUser NVARCHAR(MAX)
		, @JobCommandFailedBackups NVARCHAR(MAX)
		, @JobCommandFailedLogBackups NVARCHAR(MAX)
		, @JobCommandBackupUserLog NVARCHAR(MAX)
		, @JobStepIntegrityCheck NVARCHAR(MAX)
		, @JobStepIndexOptimize NVARCHAR(MAX)
		, @JobStepMSDBIndexOptimize NVARCHAR(MAX)
		, @JobStepCleanupCommandLog NVARCHAR(MAX)
		, @JobStepPurgeHistory NVARCHAR(MAX)
		, @JobStepCleanUpJobsHistory NVARCHAR(MAX)
		, @JobStepCleanMessages NVARCHAR(MAX)
		, @JobStepOutputTFileCleanUp NVARCHAR(MAX)
		, @JobStepOutputAFileCleanUp NVARCHAR(MAX)
		, @JobStepCheckStepsErrors NVARCHAR(MAX)
		, @JobStepStopExecution NVARCHAR(MAX)
		, @JobStepCyclingErrorLog NVARCHAR(MAX)
		, @JobStepBackupSystem NVARCHAR(MAX)
		, @JobStepBackupUser NVARCHAR(MAX)
		, @JobStepBackupUserLog NVARCHAR(MAX)
		, @OutputFileIntegrityCheck NVARCHAR(MAX)
		, @OutputFileIndexOptimize NVARCHAR(MAX)
		, @OutputFileMSDBIndexOptimize NVARCHAR(MAX)
		, @OutputFileCleanupCommandLog NVARCHAR(MAX)
		, @OutputFilePurgeHistory NVARCHAR(MAX)
		, @OutputFileCleanUpJobsHistory NVARCHAR(MAX)
		, @OutputFileCleanUpEMessages NVARCHAR(MAX)
		, @OutputFileOutputTFileCleanUp NVARCHAR(MAX)
		, @OutputFileOutputAFileCleanUp NVARCHAR(MAX)
		, @OutputFileCheckStepsErrors NVARCHAR(MAX)
		, @OutputFileStopExecution NVARCHAR(MAX)
		, @OutputFileBackupSystem NVARCHAR(MAX)
		, @OutputFileBackupUser NVARCHAR(MAX)
		, @OutputFileFailedBackups NVARCHAR(MAX)
		, @OutputFileFailedLogBackups NVARCHAR(MAX)
		, @OutputFileBackupUserLog NVARCHAR(MAX)
		, @OutputFileCyclingErrorLog NVARCHAR(MAX)
		, @JobNameOla NVARCHAR(MAX)
		, @JobScheduleID INT
		, @JobScheduleName NVARCHAR(MAX)
		, @JobCommand NVARCHAR(MAX)
		, @BackupDirectory NVARCHAR(MAX)
		, @OutputFile NVARCHAR(MAX)
		, @TokenLogDirectory NVARCHAR(MAX)
		, @OnlyModifiedStatistics NVARCHAR(MAX)
		, @MSOperator NVARCHAR(MAX)
		, @aMessage NVARCHAR(MAX)
		, @Error INT = 0
		, @Flag INT = 0
		, @ROOTKEY NVARCHAR(20)
		, @ErrorMessage NVARCHAR(MAX)
		, @sDeletedJob NVARCHAR(MAX) = 'Job "%s" already exists and cannot be changed.'
		, @sCreatedJob NVARCHAR(MAX) = 'Job "%s" created and not scheduled for execution by default.'
		, @sNoSchedule NVARCHAR(MAX) = N'This job does not scheduled for execution by default.'
		, @sNoJob NVARCHAR(MAX) = 'Job "%s" does not exists.'
		, @sStandJob NVARCHAR(MAX) = 'Job "%s" created and will be executed daily at: %s'
		, @sSetParameter NVARCHAR(MAX) = 'To proceed further with script, set parameter @DeleteMaintenanceSolution back to ''0'' (line 215).'
		, @sConfirmDelete NVARCHAR(MAX) = 'All Maintenance Solution jobs were successfully deleted. All other actions were skipped.'
		, @sConfirmCollation NVARCHAR(MAX) = 'According to Collation on Instance level, there are databases for SharePoint application.'
		, @sDeleteJobs NVARCHAR(MAX) = 'If needed, set @DeleteMaintenanceSolution to ''1'' (line 215) and run a script again to delete only job(s) created with this script. All other existing jobs will be untouched.'
		, @ePrefix NVARCHAR(4) = 'RCN_'
		, @VersionNumber VARCHAR(10)

		, @Size NCHAR(1) = 0
		, @Schedule NCHAR(2) = 0
		, @ScheduleTime NCHAR(6)

	/*
		| Added by Dmitry Spitsyn 01.09.2018
		| Make sure that all values below are correct
	*/
		, @LockTimeout INT	= 60 		-- used to limit waiting for Locks (seconds)
		, @TimeLimit INT	= 3600		-- used to limit time spent in Index Maintenance (in Seconds, 3600 = 1 hour; 7200 = 2 hours; 10800 = three hours)
		, @CreateBackupJobs BIT = 0		-- change this value to 1 if you need the backup routines (System databases | Full; User Databases | Full and Log files)
		, @DeleteMaintenanceSolution BIT = 0	-- change this value to 1 if you need to delete all existing Maintenance Jobs
	...
	*/
		...
		, @ManageMailProfileAccount BIT = 0	-- Set this value to 1 to create a new Database Mail account holding information about an SMTP account.
	...
	*/
		...
		, @SetProfileName NVARCHAR(MAX) = '<ProfileName>'		--'@ProfileName'
		, @SetAccountName SYSNAME = '<AccountName>'		--'@AccountName'
		, @SetEmailAddress NVARCHAR(128) = '<betriebspost.aus@domainname.de>'
		, @SetReplyToAddress NVARCHAR(128) = '<support@domainname.de>'
		, @SetMailServerName NVARCHAR(MAX) = '<mail.domainname.de>'
		, @SetDisplayName NVARCHAR(128) = @@SERVERNAME + ' - Central SQL Report'
	...
	*/
	...
	IF ( SELECT CONVERT(VARCHAR, SERVERPROPERTY('Collation')) ) LIKE 'Latin1_General_CI_AS_KS_WS'
		BEGIN
			SET @JobDescription = 'Maintenance Solution for Sharepoint' + CHAR(13) + CHAR(10) + 'Version: ' + @VersionNumber + CHAR(13) + CHAR(10)
			SET @TimeLimit = @TimeLimit + 3600
		END
	ELSE
		BEGIN
			SET @JobDescription = 'Maintenance Solution' + CHAR(13) + CHAR(10) + 'Version: ' + @VersionNumber + CHAR(13) + CHAR(10)
		END
	...
	*/
	SET @JobNameMaintenanceSolution = @ePrefix + 'MAINTENANCE_ALL_DATABASES'
	...
	*/
	...
	SET @JobNameCyclingErrorLog = @ePrefix + 'CYCLE_LOG'
	...
	ELSE
		BEGIN
			...
			PRINT FORMATMESSAGE(@sStandJob, @JobNameCyclingErrorLog, '06:00:00 AM')
		END
	...
	IF EXISTS (SELECT name FROM msdb.dbo.sysoperators WHERE name = @MSOperator)
		...
		BEGIN
			EXEC msdb.dbo.sp_add_operator 
				...
				, @pager_address = N''
				, @enabled = 1
			PRINT @MSOperator + ' operator were successfully created.'
		END
	SkipOperator:
	...
	*/
	...
	IF @BackupDirectory IS NOT NULL
		BEGIN
			SET @JobDescriptionWithNoSchedule = @JobDescription + @sNoSchedule + CHAR(13) + CHAR(10)
	...
	ELSE
		BEGIN
			SET @JobDescriptionWithNoSchedule = @JobDescription + @sNoSchedule
		END
	...
	*/
	...
	SET @JobCommandStopExecution = 'DECLARE @RunningJobName VARCHAR(255)' + CHAR(13) + CHAR(10)
		...
		+ 'AND sj.name LIKE ''' + @ePrefix + '%''' + CHAR(13) + CHAR(10)
	...
	SET @OutputFileStopExecution = @OutputFileDirectory + '\' + 'STOP_EXECUTION_' + @ePrefix + 'JOBS_' + @TokenDate + '_' + @TokenTime + '.txt'
	...
	/*
		| Added by Dmitry Spitsyn 01.09.2018
	*/
	SET @JobNameStopExecution = @ePrefix + 'STOP_EXECUTION_' + @ePrefix + 'JOBS'
	SET @JobDescriptionStopExecution = N'Job can be started to stop running Maintenance Job.' + CHAR(13) + CHAR(10)
		+ N'Some long-running Transact-SQL statements such as BACKUP, RESTORE, and some DBCC commands can take a long time to finish.' + CHAR(13) + CHAR(10) + @sNoSchedule
	...
	ELSE
		BEGIN
			IF EXISTS ( SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobNameStopExecution )
				...
				PRINT FORMATMESSAGE(@sCreatedJob, @JobNameStopExecution)
		END
	SkipCreateStopJob:
		...
		BEGIN
			...
			BEGIN
				...
				WHERE description LIKE '%Maintenance Solution%'
					...
					CLOSE csrjobs
				DEALLOCATE csrjobs
			PRINT FORMATMESSAGE(@sConfirmDelete) + CHAR(13) + CHAR(10) + @sSetParameter
			GOTO TheEnd
			END
			ELSE
			BEGIN
				PRINT FORMATMESSAGE(@sNoJob, @JobNameMaintenanceSolution) + CHAR(13) + CHAR(10) + @sSetParameter
				GOTO TheEnd
			END
		END
	SkipDelete:
	...
	*/
	...
	BEGIN
		PRINT FORMATMESSAGE(@sDeletedJob, @JobNameMaintenanceSolution)
		SET @Flag = 1
		GOTO SkipCreateJob
	END
	ELSE
	BEGIN
		IF NOT EXISTS ( SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobNameMaintenanceSolution )
			EXEC msdb.dbo.sp_add_job 
				...
				, @category_name = @JobCategory
				, @owner_login_name = @JobOwner
			PRINT FORMATMESSAGE(@sStandJob, @JobNameMaintenanceSolution, @sExecutionTime)
		...
		IF (SELECT CONVERT(VARCHAR, SERVERPROPERTY('Collation'))) LIKE 'Latin1_General_CI_AS_KS_WS'
			...
			IF @Size = '0' OR @Size = '2'
				EXEC msdb.dbo.sp_add_jobstep 
					...
					, @retry_interval = 10
					, @on_fail_action = 3
			PRINT FORMATMESSAGE(@sConfirmCollation)
			END
		ELSE
			BEGIN
	...
	*/
	IF @CreateBackupJobs = 1
		BEGIN
		SET @JobNameBackupSystem = @ePrefix + 'BACKUP_SYSTEM_DATABASES'
		...
		IF EXISTS ( SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobNameBackupSystem )
			BEGIN
				PRINT FORMATMESSAGE(@sDeletedJob, @JobNameBackupSystem)
		...
		ELSE
			BEGIN
				...
				PRINT FORMATMESSAGE(@sCreatedJob, @JobNameBackupSystem)
			END
		SkipCreateBSDJob:
	...
	*/
		SET @JobNameBackupUser = @ePrefix + 'BACKUP_USER_DATABASES_FULL'
	...
	*/	
		...
		IF EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobNameBackupUser)
			BEGIN
				PRINT FORMATMESSAGE(@sDeletedJob, @JobNameBackupUser)
		...
		ELSE
			BEGIN
				...
				PRINT FORMATMESSAGE(@sCreatedJob, @JobNameBackupUser)
			END
		SkipCreateBUDJob:
	...
	*/
		SET @JobNameBackupUserLog = @ePrefix + 'BACKUP_USER_DATABASES_LOG'
	...
	*/	
		...
		IF EXISTS ( SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobNameBackupUserLog )
			BEGIN
				PRINT FORMATMESSAGE(@sDeletedJob, @JobNameBackupUserLog)
		...
		ELSE
			BEGIN
				...
				PRINT FORMATMESSAGE(@sCreatedJob, @JobNameBackupUserLog)
			END
		SkipCreateBUDLJob:
		END

		IF EXISTS ( SELECT * FROM msdb.dbo.sysjobs WHERE [name] IN ( @JobNameMaintenanceSolution, @JobNameBackupSystem, @JobNameBackupUser, @JobNameBackupUserLog ))
		BEGIN
			PRINT FORMATMESSAGE(@sDeleteJobs)
		END
		...
		*/
		IF 4 <> ( SELECT COUNT(SPECIFIC_NAME)
			FROM INFORMATION_SCHEMA.ROUTINES
			WHERE ROUTINE_DEFINITION LIKE '%2020-01-26 14:06:53%' 
				AND ROUTINE_TYPE = 'PROCEDURE'
				)
				BEGIN
					SET @ErrorMessage = 'Created job(s) depends on the missing objects from ''' + 'Maintenance_Solution.sql'' and cannot be run successfully until all procedures (Version: ' + @VersionNumber + ') are installed.'
